...
sh -c "
  cd /tmp;
  git clone https://github.com/platform-dx/backstage-helm-chart.git backstage
"
...
kubectl wait deploy/backstage-frontend --namespace=default --for condition=Available=True --timeout=60s
kubectl wait deploy/backstage-backend --namespace=default --for condition=Available=True --timeout=60s
...
kubectl wait deploy/backstage-lighthouse --namespace=default --for condition=Available=True --timeout=60s
...
# kubectl wait deploy/grafana --namespace=monitoring --for condition=Available=True --timeout=60s
# kubectl create ingress grafana --namespace=monitoring --rule="grafana.${DOMAIN}/*=grafana:3000" --class=public
# kubectl wait deploy/prometheus-operator --namespace=monitoring --for condition=Available=True --timeout=60s
